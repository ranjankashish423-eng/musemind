...
(function () {
  ...
  const EMOTION_CONFIDENCE_THRESHOLD = 0.20;
  const MIXED_EMOTION_THRESHOLD = 0.35;

  function normalizeGoLabel(label) {
    const raw = String(label || '').trim().toLowerCase();
    if (GO_EMOTIONS.includes(raw)) return raw;
    const match = raw.match(/^(?:label[_-]?)?(\d+)$/);
    if (match) {
      const index = Number(match[1]);
      return GO_EMOTIONS[index] || raw;
    }
    return raw;
  }
  ...
  function normalizeModelOutput(output) {
    ...
    return (Array.isArray(raw) ? raw : [])
      .filter((x) => x && x.label != null && Number.isFinite(Number(x.score)))
      .map((x) => ({
        label: normalizeGoLabel(x.label),
        confidence: Math.max(0, Math.min(1, Number(x.score)))
      }))
      .filter((x) => GO_EMOTIONS.includes(x.label))
      .sort((a, b) => b.confidence - a.confidence);
  }

  function getMeaningfulGoResults(goResults) {
    const meaningful = goResults.filter(
      (x) => x.label !== 'neutral' && x.confidence >= EMOTION_CONFIDENCE_THRESHOLD
    );
    return meaningful.length ? meaningful : goResults.filter((x) => x.label !== 'neutral').slice(0, 1);
  }

  function categoryScoresFromGoResults(goResults) {
    const categoryScores = {};
    Object.keys(EMOTIONS).forEach((e) => { categoryScores[e] = 0; });
    goResults.forEach((item) => {
      const category = GO_TO_MUSE[item.label];
      if (category) categoryScores[category] = Math.max(categoryScores[category], item.confidence);
    });
    return categoryScores;
  }

  function chooseCategoryPair(categoryScores) {
    const sorted = Object.entries(categoryScores)
      .filter(([, score]) => score > 0)
      .sort((a, b) => b[1] - a[1]);
    if (!sorted.length) return { primary: 'Calm', secondary: 'Calm' };
    return {
      primary: sorted[0][0],
      secondary: sorted.find(([name]) => name !== sorted[0][0])?.[0] || sorted[0][0]
    };
  }
  ...
  const EmotionEngine = {
    ...
    async loadModel() {
      ...
      modelPromise = transformersModulePromise.then(({ pipeline }) =>
        pipeline('text-classification', MODEL_ID, { dtype: 'q8' })
      )
        .then((classifier) => {
    ...
    async classifyModelText(text) {
      const classifier = await this.loadModel();
      const output = await classifier(text, { top_k: null });
      const results = normalizeModelOutput(output);
      if (!results.length) throw new Error('GoEmotions returned no recognized labels.');
      return results;
    },

    mapScores(goResults) {
      return categoryScoresFromGoResults(goResults);
    },
    ...
    async analyze(text, language) {
      ...
      if (!englishEligible) {
        ...
        return Object.assign(fallback, {
          ...
          goEmotionScores: [],
          primaryGoLabel: null,
          secondaryGoLabel: null
        });
      }

      try {
        const goResults = await this.classifyModelText(text);
        const meaningful = getMeaningfulGoResults(goResults);
        const primaryGo = meaningful[0] || goResults[0];
        const secondaryGo = meaningful[1] || goResults.find((x) => x.label !== primaryGo?.label);
        const categoryScores = this.mapScores(goResults);
        const pair = chooseCategoryPair(categoryScores);
        const primaryPercent = Math.round((categoryScores[pair.primary] || 0) * 100);
        const secondaryPercent = pair.secondary === pair.primary
          ? 0
          : Math.round((categoryScores[pair.secondary] || 0) * 100);
        const mixed = meaningful.filter((x) => x.confidence >= MIXED_EMOTION_THRESHOLD).length >= 2;
        const lowerText = text.toLowerCase();
        const triggers = detectTriggers(lowerText);
        const emojiDetected = EMOJI_RANGE.test(text);
        const moodScore = Math.round(
          (MOOD_SCORE_MAP[pair.primary] * Math.max(primaryPercent, 1) +
           MOOD_SCORE_MAP[pair.secondary] * Math.max(secondaryPercent, 0)) /
          Math.max(primaryPercent + secondaryPercent, 1)
        );

        // Run the same real transformer independently on each sentence.
        const rawSentences = splitSentences(text);
        const sentenceResults = await Promise.all(rawSentences.map(async (sentence) => {
          try {
            const sentenceScores = await this.classifyModelText(sentence);
            const sentenceMeaningful = getMeaningfulGoResults(sentenceScores);
            const top = sentenceMeaningful[0] || sentenceScores[0];
            return {
              text: sentence,
            ...
            const fallback = analyzeRuleBased(sentence);
            return {
              text: sentence,
              emotion: fallback.primary,
              goLabel: null,
              confidence: fallback.primaryPercent / 100,
        ...
        return {
          ...
          primaryPercent,
          secondaryPercent,
          primaryGoLabel: primaryGo?.label || null,
          secondaryGoLabel: secondaryGo?.label || null,
          primaryGoConfidence: primaryGo?.confidence || 0,
          secondaryGoConfidence: secondaryGo?.confidence || 0,
          mixed,
          triggers,
          language,
          emojiDetected,
          ...
          // Keep ALL 28 real model outputs. goEmotions is the display subset.
          goEmotionScores: goResults,
          goEmotions: meaningful.slice(0, 8),
          modelUsed: MODEL_ID,
  ...
  async function analyzeEmotion() {
    ...
    try {
      ...
      const entry = {
        ...
        goEmotionScores: result.goEmotionScores || result.goEmotions || [],
        primaryGoLabel: result.primaryGoLabel || null,
        secondaryGoLabel: result.secondaryGoLabel || null,
        primaryGoConfidence: result.primaryGoConfidence || 0,
        secondaryGoConfidence: result.secondaryGoConfidence || 0,
  ...
  function showAnalysis(entry, previousEntries) {
    ...
    setText('primaryEmoji', EMOTIONS[safePrimary].emoji);
    setText('primaryEmotion', safePrimary);
    setText('primaryPercent', `${primaryPct}%`);
    setText('primaryPercentRadial', `${primaryPct}%`);
    setWidth('primaryProgress', primaryPct);
    const pr = document.getElementById('primaryRadial'); if (pr) pr.style.setProperty('--score', primaryPct);
    setText('primaryCategory', safePrimary);

    setText('secondaryEmoji', EMOTIONS[safeSecondary].emoji);
    setText('secondaryEmotion', safeSecondary);
    setText('secondaryPercent', `${secondaryPct}%`);
    setText('secondaryPercentRadial', `${secondaryPct}%`);
    setText('secondaryCategoryLine', `MuseMind Category: ${safeSecondary}`);
    const sr = document.getElementById('secondaryRadial'); if (sr) sr.style.setProperty('--score', secondaryPct);

    setText('analysisModel', entry.modelUsed || 'MuseMind Rule-Based Emotion Engine');
    setText('analysisSource', entry.analysisSource || 'Legacy analysis');
    setText('analysisLanguage', entry.language || 'Unknown');
    setText('analysisTimestamp', entry.timestamp ? `${fmtDate(new Date(entry.timestamp))} • ${entry.time || ''}` : '—');

    const banner = document.getElementById('analysisEngineBanner');
    if (banner) {
      banner.innerHTML = entry.analysisSource === 'GoEmotions Model'
        ? '🟢 <span><strong>STANDARD NLP MODEL</strong> · GoEmotions Transformer</span>'
        : '🟡 <span><strong>FALLBACK ENGINE</strong> · MuseMind Rule-Based Detection</span>';
    }
    ...
    setText('analysisMeta', `${entry.language || 'Unknown'} • ${entry.wordCount || 0} words${entry.emojiDetected ? ' • emoji detected' : ''}`);

    const allGo = (entry.goEmotionScores && entry.goEmotionScores.length)
      ? entry.goEmotionScores
      : (entry.goEmotions || []);
    const meaningful = allGo.filter((x) => x.label !== 'neutral' && Number(x.confidence) >= EMOTION_CONFIDENCE_THRESHOLD)
      .sort((a, b) => b.confidence - a.confidence);

    const ranking = document.getElementById('emotionRanking');
    if (ranking) {
      ranking.innerHTML = meaningful.length
        ? meaningful.slice(0, 8).map((x) => {
            const pct = Math.round(x.confidence * 100);
            const category = GO_TO_MUSE[x.label] || '—';
            return `<div class="emotion-ranking-row"><div><strong>${escapeHTML(x.label)}</strong><small>${escapeHTML(category)}</small></div><b>${pct}%</b><div class="ranking-bar"><i style="width:${Math.max(2,pct)}%"></i></div></div>`;
          }).join('')
        : '<p class="muted-note">No standard model scores are available for this entry. The fallback engine was used.</p>';
    }

    const standardBox = document.getElementById('standardEmotionList');
    if (standardBox) {
      standardBox.innerHTML = meaningful.length
        ? meaningful.slice(0, 8).map(x => `<div class="standard-emotion-item"><div class="label">${escapeHTML(x.label)}</div><div class="score">${Math.round(x.confidence*100)}%</div><div class="bar"><i style="--w:${Math.max(2,Math.round(x.confidence*100))}%"></i></div></div>`).join('')
        : '<div class="muted-note">Standard model labels unavailable; fallback engine used.</div>';
    }
    const sourceBadge = document.getElementById('modelSourceBadge');
    if (sourceBadge) sourceBadge.textContent = `Source: ${entry.analysisSource || 'Legacy'} · Model: ${entry.modelUsed || 'Legacy rule engine'}`;

    const mixedBox = document.getElementById('mixedEmotionContent');
    if (mixedBox) {
      mixedBox.innerHTML = entry.mixed && meaningful.length >= 2
        ? `<div class="mixed-emotions">${meaningful.filter(x => x.confidence >= MIXED_EMOTION_THRESHOLD).slice(0,5).map(x => `<span class="chip">${escapeHTML(x.label)} · ${Math.round(x.confidence*100)}% · ${escapeHTML(GO_TO_MUSE[x.label] || '—')}</span>`).join('')}</div><p class="tiny-note">Multiple meaningful emotion signals passed the mixed-emotion threshold.</p>`
        : '<p class="muted-note">No mixed emotion signal detected above the configured threshold.</p>';
    }

    const mappingBox = document.getElementById('mappingContent');
    if (mappingBox) {
      mappingBox.innerHTML = meaningful.slice(0,6).map(x => `<div class="mapping-row"><span>${escapeHTML(x.label)}</span><b>→</b><strong>${escapeHTML(GO_TO_MUSE[x.label] || 'Unmapped')}</strong></div>`).join('') || '<p class="muted-note">Mapping will appear after a model analysis.</p>';
    }

    renderSpectrum(entry, meaningful);
    renderEmotionRadar(entry);
    renderSentenceAnalysis(entry);
    renderJourney(entry);
    renderEvidence(entry);
    renderLanguagePanel(entry);
    renderMoodPanel(entry);
    renderHistoryComparisons(entry, previousEntries || []);
    renderBaseline(entry, previousEntries || []);
    renderReflectionPrompts(entry);

    setText('reflectionText', entry.reflection || 'Your reflection will appear here after analysis.');
    const sentBox = document.getElementById('sentenceBreakdown');
    if (sentBox) sentBox.style.display = (entry.sentences || []).length > 1 ? 'block' : 'block';

    renderMoodMirror(entry, previousEntries && previousEntries[0]);
  }

  function renderSpectrum(entry, meaningful) {
    const positive = ['joy','amusement','love','gratitude','admiration','approval','caring','excitement','desire','pride','optimism','relief'];
    const negative = ['sadness','grief','disappointment','remorse','anger','annoyance','disgust','disapproval','fear','nervousness','embarrassment','confusion'];
    let score = 50;
    let label = entry.primaryEmotion || 'Calm';
    if (meaningful.length) {
      let weighted = 0, total = 0;
      meaningful.slice(0,8).forEach(x => {
        const direction = positive.includes(x.label) ? 1 : negative.includes(x.label) ? -1 : 0;
        weighted += direction * x.confidence;
        total += x.confidence;
      });
      score = total ? Math.round(50 + (weighted / total) * 50) : 50;
      label = meaningful[0].label;
    } else {
      score = MOOD_SCORE_MAP[entry.primaryEmotion] || 50;
    }
    const marker = document.getElementById('spectrumMarker');
    if (marker) marker.style.left = `${Math.max(2, Math.min(98, score))}%`;
    const text = document.getElementById('spectrumText');
    if (text) text.textContent = `${label} signal · ${score}/100 on the descriptive spectrum`;
  }

  function renderEmotionRadar(entry) {
    const canvas = document.getElementById('emotionRadarChart');
    if (!canvas || typeof Chart === 'undefined') return;
    const scores = {};
    GO_EMOTIONS.forEach(label => scores[label] = 0);
    (entry.goEmotionScores || entry.goEmotions || []).forEach(x => {
      if (scores[x.label] !== undefined) scores[x.label] = Number(x.confidence) || 0;
    });
    if (charts.emotionRadar) charts.emotionRadar.destroy();
    charts.emotionRadar = new Chart(canvas, {
      type: 'radar',
      data: { labels: GO_EMOTIONS, datasets: [{ label: 'Model probability', data: GO_EMOTIONS.map(x => Math.round(scores[x]*100)), fill: true, borderWidth: 1 }] },
      options: { responsive:true, maintainAspectRatio:false, scales:{ r:{ beginAtZero:true, min:0, max:100, ticks:{ display:false } } }, plugins:{ legend:{ display:false } } }
    });
  }

  function renderSentenceAnalysis(entry) {
    const list = document.getElementById('sentenceList');
    if (!list) return;
    const sentences = entry.sentences || [];
    list.innerHTML = sentences.length
      ? sentences.map((s,i) => `<div class="sentence-row"><span class="sentence-num">${i+1}</span><span class="sentence-text">${escapeHTML(s.text)}</span><span class="sentence-emotion">${EMOTIONS[s.emotion]?.emoji || '•'} ${escapeHTML(s.goLabel || s.emotion)} · ${Math.round((s.confidence || 0)*100)}%</span></div>`).join('')
      : '<p class="muted-note">No sentence-level signals available.</p>';
  }

  function renderJourney(entry) {
    const box = document.getElementById('emotionJourney');
    if (!box) return;
    const sentences = (entry.sentences || []).filter(s => s.emotion);
    if (sentences.length < 2) {
      box.innerHTML = '<p class="muted-note">Not enough sentence-level signals to determine an emotion journey.</p>';
      return;
    }
    box.innerHTML = sentences.map((s,i) => `<div class="journey-step"><small>${i===0?'START':i===sentences.length-1?'FINAL':`STEP ${i+1}`}</small><strong>${EMOTIONS[s.emotion]?.emoji || '•'} ${escapeHTML(s.goLabel || s.emotion)}</strong><span>${Math.round((s.confidence || 0)*100)}%</span></div>`).join('<div class="journey-arrow">↓</div>');
  }

  function renderEvidence(entry) {
    const box = document.getElementById('textEvidence');
    if (!box) return;
    let html = escapeHTML(entry.text || '');
    const words = new Set();
    Object.values(EMOTIONS).forEach(def => def.words.forEach(w => words.add(String(w).toLowerCase())));
    (entry.triggers || []).forEach(t => (TRIGGERS[t] || []).forEach(w => words.add(String(w).toLowerCase())));
    [...words].filter(w => w.length > 2).sort((a,b)=>b.length-a.length).slice(0,35).forEach(word => {
  ...
  function renderLanguagePanel(entry) {
    const box = document.getElementById('languageAnalysis');
    if (!box) return;
    box.innerHTML = `<div><strong>Detected Language</strong><span>${escapeHTML(entry.language || 'Unknown')}</span></div><div><strong>Analysis Engine</strong><span>${entry.analysisSource === 'GoEmotions Model' ? 'GoEmotions Transformer' : 'MuseMind Rule-Based Fallback'}</span></div><div><strong>Model</strong><span>${escapeHTML(entry.modelUsed || 'Fallback')}</span></div>`;
  }

  function renderMoodPanel(entry) {
    const score = Number(entry.moodScore) || 0;
    const el = document.getElementById('analysisMoodScore');
    if (el) el.textContent = score;
    const box = document.getElementById('moodContributions');
    if (box) box.innerHTML = `<div><span>Primary contribution</span><b>${escapeHTML(entry.primaryEmotion)} · ${entry.primaryPercentage || 0}%</b></div><div><span>Secondary contribution</span><b>${escapeHTML(entry.secondaryEmotion)} · ${entry.secondaryPercentage || 0}%</b></div><div><span>Indicator type</span><b>Descriptive only</b></div>`;
  }

  function renderHistoryComparisons(entry, previousEntries) {
    const previous = previousEntries && previousEntries[0];
    const deltaBox = document.getElementById('emotionDelta');
    if (!deltaBox) return;
    if (!previous) {
      deltaBox.textContent = 'Add another journal entry to unlock emotion delta.';
      return;
    }
    const current = entry.scores || {};
    const prev = previous.scores || {};
    const rows = Object.keys(EMOTIONS).map(name => ({ name, delta: (current[name] || 0) - (prev[name] || 0) }))
      .sort((a,b)=>Math.abs(b.delta)-Math.abs(a.delta)).slice(0,4);
    deltaBox.innerHTML = rows.map(r => `<div class="delta-row"><span>${EMOTIONS[r.name].emoji} ${r.name}</span><strong class="${r.delta>=0?'delta-up':'delta-down'}">${r.delta>=0?'+':''}${Math.round(r.delta*100)}%</strong></div>`).join('');
  }

  function renderBaseline(entry, previousEntries) {
    const box = document.getElementById('personalBaseline');
    const unusual = document.getElementById('unusualForYou');
    if (!box) return;
    const history = previousEntries || [];
    if (!history.length) {
      box.innerHTML = '<p class="muted-note">Add more entries to build your personal baseline.</p>';
      if (unusual) unusual.textContent = 'Not enough history yet.';
      return;
    }
    const avgMood = Math.round(history.reduce((s,e)=>s+(Number(e.moodScore)||0),0)/history.length);
    box.innerHTML = `<div class="baseline-row"><span>Recent average mood</span><strong>${avgMood}/100</strong></div><div class="baseline-row"><span>Entries compared</span><strong>${history.length}</strong></div><div class="baseline-row"><span>Current mood</span><strong>${entry.moodScore}/100</strong></div>`;
    if (unusual) unusual.textContent = Math.abs((entry.moodScore||0)-avgMood) >= 15 ? `This entry is ${Math.abs(entry.moodScore-avgMood)} points ${entry.moodScore>=avgMood?'above':'below'} your recent average.` : 'This entry is within your recent mood range.';
  }
  ...
  function renderReflectionPrompts(entry) {
    ...
    if (!box) return;
    const prompts = {
      Happy: 'What happened today that you would like to experience again?', Sad: 'What do you think is behind this feeling?', Stressed: 'What is one small thing you can take off your mind today?', Calm: 'What helped you feel settled today?', Excited: 'What are you most looking forward to?', Angry: 'What part of the situation felt most frustrating?', Anxious: 'What is within your control right now?', Confident: 'What did you do that made you feel capable?'
    };
    box.innerHTML = `<span>${escapeHTML(prompts[entry.primaryEmotion] || 'What would you like to understand better about this moment?')}</span>`;
  }

  function showMappingLogic() {
    const rows = Object.entries(GO_TO_MUSE).map(([go,muse]) => `<div class="mapping-row"><span>${escapeHTML(go)}</span><b>→</b><strong>${escapeHTML(muse)}</strong></div>`).join('');
    const box = document.getElementById('mappingContent');
    if (box) box.innerHTML = rows;
    toast('Showing the full GoEmotions → MuseMind mapping.', 'success');
  }

  function togglePBLMode() {
    const panel = document.getElementById('pblPanel');
    if (!panel) return;
    panel.style.display = panel.style.display === 'none' ? 'block' : 'none';
  }

  function exportCurrentAnalysis() {
    if (!currentAnalysisEntry) { toast('Analyze a journal entry first.', 'warn'); return; }
    const blob = new Blob([JSON.stringify(currentAnalysisEntry, null, 2)], {type:'application/json'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `musemind-analysis-${currentAnalysisEntry.id}.json`; a.click();
  ...
  function entryCardHTML(entry) {
    ...
    return `<div class="entry-card" data-id="${entry.id}">
      <div class="entry-card-top">
        <span class="entry-emotion">${EMOTIONS[entry.primaryEmotion]?.emoji || '•'} ${entry.primaryEmotion} ${entry.primaryPercentage > 0 ? entry.primaryPercentage + '%' : '—'}${entry.primaryGoLabel ? ` · ${escapeHTML(entry.primaryGoLabel)}` : ''}</span>
  ...
  function viewEntry(id) {
    ...
    document.getElementById("entryModalBody").innerHTML = `
      ...
      <div class="modal-tags">
        <span class="chip">${EMOTIONS[entry.primaryEmotion]?.emoji || '•'} ${entry.primaryEmotion} ${entry.primaryPercentage > 0 ? entry.primaryPercentage + '%' : '—'}</span>
        <span class="chip">${EMOTIONS[entry.secondaryEmotion]?.emoji || '•'} ${entry.secondaryEmotion} ${entry.secondaryPercentage > 0 ? entry.secondaryPercentage + '%' : '—'}</span>
        ${entry.primaryGoLabel ? `<span class="chip chip-muted">Standard: ${escapeHTML(entry.primaryGoLabel)} ${Math.round((entry.primaryGoConfidence||0)*100)}%</span>` : ''}
        ${entry.triggers.map((t) => `<span class="chip chip-muted">🔎 ${t}</span>`).join("")}
      </div>
      <p class="modal-reflection">${escapeHTML(entry.reflection || "")}</p>
      <button class="mini-btn" onclick="MuseMind.reanalyzeEntry('${entry.id}')">Re-analyze with current model</button>
    `;
  ...
  async function reanalyzeEntry(id) {
    ...
    updated.reflection = generateReflection(updated, DataLayer.getUserEntries(entry.userId).filter(e => e.id !== entry.id).slice(0,10));
    DataLayer.updateEntry(entry.id, updated);
    showAnalysis(updated, DataLayer.getUserEntries(entry.userId).filter(e => e.id !== entry.id).sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,10));
    closeEntryModal();
    goToSection('analysis');
    toast('Entry re-analyzed with the current emotion engine.', 'success');
  }
  ...
  async function loadDemoData() {
    const user = DataLayer.getCurrentUser();
    if (!user) return;
    for (const d of DEMO_TEXTS) {
      const date = new Date();
      date.setDate(date.getDate() - d.offset);
      const result = await analyzeText(d.text);
      const entry = {
        id: uid("demo"),
        userId: user.id,
        text: d.text,
